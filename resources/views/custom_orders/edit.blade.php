...
@section('content')
    <div class="card bg-info-subtle shadow-none position-relative overflow-hidden mb-4">
        <div class="card-body px-4 py-3">
            <div class="row align-items-center">
                <div class="col-9">
                    <h4 class="fw-semibold mb-8">Edit Pesanan Khusus {{$corder->code}}</h4>
    ...
    <div class="row">
        <div class="col-md-4 h-100 mb-3">
            <div id="placeholder-raw" class="mb-4 {{$corder->lampiran_bahan ? '' : 'd-none'}}">
                <h6>Preview Lampiran Bahan Baku</h6>
                <div id="placeholder-image-raw"
                    class="{{$corder->lampiran_bahan ? 'd-none' : ''}} d-flex p-5 text-center align-items-center justify-content-center rounded-5 border-2 border-dashed"
                    style="aspect-ratio:1/1">
                    <div>
                        <div class="fs-4">If Image / File Selected, it will show (Preview)</div>
                    </div>
                </div>
                <img src="{{$corder->lampiran_bahan ? '/storage/'.$corder->lampiran_bahan : ''}}" id="preview-image-raw" alt="Bahan Baku Image Preview" class="d-none rounded-4 shadow w-100"
                    style="">
                <div id="preview-file-raw" class="{{$corder->lampiran_bahan ? '' : 'd-none'}}">
                    <a href="{{$corder->lampiran_bahan ? '/storage/'.$corder->lampiran_bahan : ''}}" id="preview-file-link-raw" class="d-flex align-items-center gap-2 p-2 rounded-2 border border-al-primary" target="_blank">
                        <i class="ti ti-file fs-6"></i>
                        <div id="preview-file-text-raw" class="fs-2 line-clamp line-clamp-2">{{ $corder->lampiran_bahan ? $corder->lampiran_bahan : 'Tidak ada'}}</div>
                    </a>
                    <embed id="preview-file-embed-raw" src="" class="rounded-4 overflow-hidden mt-3 {{$corder->lampiran_bahan ? 'd-none' : ''}}" width="100%" height="600px" type="application/pdf">
                </div>
            </div>
            <div id="placeholder" class="mb-4 {{$corder->lampiran ? '' : 'd-none'}}">
                <h6>Preview Lampiran</h6>
                <div id="placeholder-image"
                    class="{{$corder->lampiran ? 'd-none' : ''}} d-flex p-5 text-center align-items-center justify-content-center rounded-5 border-2 border-dashed"
                    style="aspect-ratio:1/1">
                    <div>
                        <div class="fs-4">If Image / File Selected, it will show (Preview)</div>
                    </div>
                </div>
                <img src="{{$corder->lampiran ? '/storage/'.$corder->lampiran : ''}}" id="preview-image" alt="Bahan Baku Image Preview" class="d-none rounded-4 shadow w-100"
                    style="">
                <div id="preview-file" class="{{$corder->lampiran ? '' : 'd-none'}}">
                    <a href="{{$corder->lampiran ? '/storage/'.$corder->lampiran : ''}}" id="preview-file-link" class="d-flex align-items-center gap-2 p-2 rounded-2 border border-al-primary" target="_blank">
                        <i class="ti ti-file fs-6"></i>
                        <div id="preview-file-text" class="fs-2 line-clamp line-clamp-2">{{ $corder->lampiran ? $corder->lampiran : 'Tidak ada'}}</div>
                    </a>
                    <embed id="preview-file-embed" src="" class="rounded-4 overflow-hidden mt-3 {{$corder->lampiran ? 'd-none' : ''}}" width="100%" height="600px" type="application/pdf">
                </div>
            </div>
        </div>
        <div class="col-md-8">
            <div class="card">
                <div class="px-4 py-3 border-bottom">
                    <h5 class="card-title fw-semibold mb-0">Edit Pesanan Khusus</h5>
                </div>
                <div class="card-body p-4">
                    ...
                    @endif
                    <form action="{{ route('custom-order.update', $corder->id) }}" method="POST" enctype="multipart/form-data">
                        @csrf
                        <div class="mb-4">
                            <div class="form-check form-switch">
                                <input class="form-check-input" name="raw_from_customer" type="checkbox" value="1" id="raw_from_customer" {{$corder->cek_bahan_dari_pelanggan ? 'checked' : ''}} />
                                <label class="form-check-label" for="raw_from_customer">Apakah Bahan Baku dari Anda Sendiri ?</label>
                            </div>
                        </div>
                        <div id="raw-from-owner" class="p-4 bg-light rounded-4 {{$corder->cek_bahan_dari_pelanggan ? 'd-none' : ''}}">
                                ...
                                <div style="flex-grow:1">
                                    <select name="raw_material_id" id="raw_material_id" class="select2 form-select">
                                        ...
                                        @foreach ($raw_materials as $raw_material)
                                            <option value="{{$raw_material->id}}" {{$raw_material->id == old('raw_material_id', $corder?->bahan_baku_id) ? 'selected' : ''}}>{{$raw_material->nama}}</option>
                                        @endforeach
                                    </select>
                                </div>
                            </div>
                        ...
                        <div id="raw-from-customer" class="{{$corder->cek_bahan_dari_pelanggan ? '' : 'd-none'}} p-4 rounded-4 bg-light">
                            <div class="mb-4">
                                <label class="form-label fw-semibold">Nama Bahan Baku yang anda Bawa</label>
                                <div class="input-group">
                                    <span class="input-group-text px-6" id="basic-addon1"><i
                                            class="ti ti-text-caption fs-6"></i></span>
                                    <input type="text" name="raw_name" class="form-control bg-white ps-2" value="{{old('raw_name', $corder->nama_bahan)}}" placeholder="Nama Bahan Baku">
                                </div>
                                @error('raw_name')
                                    <span class="invalid-feedback" role="alert">
                                        {{ $message }}
                            ...
                            <div class="mb-4">
                                ...
                                <div class="input-group">
                                    ...
                                    <textarea class="form-control bg-white ps-2" name="raw_description" id="raw_description" cols="20" rows="5"
                                        placeholder="Jelaskan Bahan Baku milik Anda yang akan digunakan">{{old('raw_description', $corder->keterangan_bahan)}}</textarea>
                                </div>
                                ...
                                <div style="flex-grow:1">
                                    <select name="product_id" id="product_id" class="select2 form-select">
                                        ...
                                        @foreach ($products as $product)
                                            <option value="{{$product->id}}" {{$product->id == old('product_id', $corder->produk_id) ? 'selected' : ''}}>{{$product->nama}}</option>
                                        @endforeach
                                    </select>
                                </div>
                            </div>
                        ...
                        <div class="mb-4">
                            ...
                            <div class="input-group">
                                ...
                                <input type="number" name="qty" class="form-control ps-2" value="{{old('qty', $corder->qty)}}" placeholder="Buat Produk pada permintaan produksi ini sebanyak ... pcs">
                            </div>
                            @error('qty')
                                <span class="invalid-feedback" role="alert">
                                    {{ $message }}
                        ...
                        <div class="mb-4">
                            ...
                            <div class="input-group">
                                ...
                                <input type="date" name="deadline" class="form-control ps-2" value="{{old('deadline', $corder->deadline)}}">
                            </div>
                            @error('deadline')
                                <span class="invalid-feedback" role="alert">
                                    {{ $message }}
                        ...
                        <div class="mb-4">
                            ...
                            <div class="input-group">
                                ...
                                <textarea class="form-control bg-white ps-2" name="keterangan" id="keterangan" cols="20" rows="5"
                                    placeholder="Tambahkan keterangan untuk pesanan khusus">{{old('keterangan', $corder->keterangan)}}</textarea>
                            </div>
                            @error('keterangan')
                                <span class="invalid-feedback" role="alert">
                                    {{ $message }}
                                ...
                                <div style="flex-grow:1">
                                    <select name="customer_id" id="customer_id" class="select2 form-select">
                                        ...
                                        @foreach ($customers as $customer)
                                            <option value="{{$customer->id}}" {{$customer->id == old('customer_id', $corder->user_id) ? 'selected' : ''}}>{{$customer->name .' | '.$customer->email}}</option>
                                        @endforeach
                                    </select>
                                </div>
                            </div>

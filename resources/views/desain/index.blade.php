...
@section('content')
    <div class="card bg-info-subtle shadow-none position-relative overflow-hidden mb-4">
        <div class="card-body px-4 py-3">
            <div class="row align-items-center">
                <div class="col-9">
                    <h4 class="fw-semibold mb-8">Desain / Konsep</h4>
                    <nav aria-label="breadcrumb">
                        <ol class="breadcrumb">
                            <li class="breadcrumb-item">
                                <a class="text-muted text-decoration-none" href="{{ route('home') }}">Dashboard</a>
                            </li>
                            <li class="breadcrumb-item" aria-current="page">Manage Desain / Konsep</li>
                        </ol>
                    </nav>
                </div>
                <div class="col-3">
    ...
    <div class="mb-3 d-flex align-items-center gap-2 justify-content-between">
        ...
        <button class="btn btn-primary btn-al-primary"
            data-bs-toggle="modal" data-bs-target="#addDesainModal"
        >Tambah</button>
    ...
    <div class="card">
        <div class="card-body">
            <form action="{{route('desain.index')}}" method="GET">
                <div class="row align-items-end mb-3 flex-wrap">
                    ...
                    <div class="col-md-3 mb-2">
                        ...
                        <select name="field" id="field" class="form-select">
                            @foreach (getModelAttributes('Desain', ['user_id', 'lampiran']) as $atr)
            ...
            <div class="table-responsive">
                <table class="table border text-nowrap mb-0 align-middle">
                    <thead class="text-dark fs-4">
                        <tr>
                            <th>
                                <h6 class="fs-3 fw-semibold mb-0">Desain</h6>
                            </th>
                            <th>
                    ...
                    <tbody>
                        @forelse ($desains as $desain)
                            <tr>
                                <td>
                                    <div class="fw-semibold fs-3 mb-2">
                                        {{ $desain->nama }}
                                    </div>
                                    <a href="{{$desain->lampiran ? '/storage/'.$desain->lampiran : '#'}}" target="_blank" class="btn btn-sm btn-al-primary d-flex text-decoration-none align-items-center gap-2">
                                        <i class="ti ti-file fs-2"></i>
                                        Lampiran Konsep
                                    </a>
                                </td>
                                <td>
                                    <div style="max-width: 30em; white-space:normal !important">{{ $desain->keterangan }}</div>
                                </td>
                                <td>
                                    <div class="d-flex flex-column align-items-start gap-2">
                                        <div class="badge bg-success-subtle text-success rounded-3 fw-semibold fs-2">Updated
                                            at
                                            : {{ $desain->updated_at }}</div>
                                        <div class="badge bg-primary-subtle text-primary rounded-3 fw-semibold fs-2">Created
                                            at
                                            : {{ $desain->created_at }}</div>
                                    </div>
                                </td>
                                <td>
                                    <div class="dropdown dropstart">
                                        ...
                                        <ul class="dropdown-menu" aria-labelledby="dropdownMenuButton">
                                            <li>
                                                <a href="{{route('desain.index', $desain->stock?->id)}}" class="dropdown-item d-flex align-items-center gap-3"><i
                                                        class="fs-4 ti ti-eye"></i>View</a>
                                            </li>
                                            <li>
                                                <button type="button" class="dropdown-item d-flex align-items-center gap-3"
                                                    data-bs-toggle="modal" data-bs-target="#editModal-{{$desain->id}}"><i
                                                        class="fs-4 ti ti-pencil"></i>Edit Desain</button>
                                            </li>
                                            <li>
                                                <button type="button" class="dropdown-item d-flex align-items-center gap-3"
                                                    data-bs-toggle="modal" data-bs-target="#deleteModal-{{$desain->id}}"><i
                                    ...
                                    <!-- Edit Modal -->
                                    <div class="modal fade" id="editModal-{{$desain->id}}" tabindex="-1" aria-labelledby="vertical-center-modal"
                                        aria-hidden="true">
                                            ...
                                            <div class="modal-content">
                                                ...
                                                <div class="modal-body">
                                                    <form action="{{ route('desain.update', $desain->id) }}" method="POST" enctype="multipart/form-data">
                                                        @csrf
                                                        <a href="{{$desain->lampiran ? '/storage/'.$desain->lampiran : '#'}}" target="_blank" class="btn mb-2 bg-primary-subtle text-primary d-flex text-decoration-none align-items-center gap-2">
                                                            <i class="ti ti-file fs-2"></i>
                                                            Lampiran Konsep
                                                        </a>
                                                        <div class="mb-2">
                                                        ...
                                                        <input type="text" name="nama" class="form-control mb-2" value="{{ old('nama', $desain->nama) }}"
                                                        ...
                                                        placeholder="Keterangan / Penjelasan konsep desain">{{ old('keterangan', $desain->keterangan) }}</textarea>
                                                        ...
                                                        <div class="d-flex gap-1 align-items-center justify-content-end">
                                                            ...
                                                            <button type="submit" class="btn btn-primary btn-al-primary">Tambah Desain</button>
                                                        </div>
                                                    </form>
                                                </div>
                                            </div>
                                    ...
                                    <!-- Delete Modal -->
                                    <div id="deleteModal-{{$desain->id}}" class="modal fade" tabindex="-1"
                                            ...
                                            <div class="modal-content p-3 modal-filled bg-danger">
                                                <div class="modal-header modal-colored-header text-white">
                                                    <h4 class="modal-title text-white" id="danger-header-modalLabel">
                                                        Yakin ingin menghapus Desain ?
                                                    </h4>
                                                    <button type="button" class="btn-close btn-close-white"
                                                        data-bs-dismiss="modal" aria-label="Close"></button>
                                                </div>
                                                <div class="modal-body" style="width: fit-content; white-space:normal">
                                                    <h5 class="mt-0 text-white">Desain {{$desain->title}} akan dihapus</h5>
                                                    <p class="text-white">Segala data yang berkaitan dengan Desain tersebut juga akan dihapus secara permanen.</p>
                                                </div>
                                                <div class="modal-footer">
                                                    <button type="button" class="btn btn-light" data-bs-dismiss="modal">
                                                        Close
                                                    </button>
                                                    <form action="{{route('desain.destroy', $desain->id)}}" method="POST">
                                                        @csrf
                                                        @method('delete')
